    ...
    for p in ps:
        ...
        if label and text:
            ...
            if labels and text:
                ...
                labeled_provision = LabeledProvision(
                    text, labels, contract_uri)
            ...
            for folder in os.listdir(year_q_dir):
                ...
                for fname in os.listdir(os.path.join(year_q_dir, folder)):
                    ...
                    if fname.endswith('.htm'):
                        ...
                        provisions_doc = scrape_exhibit_10(
                            html_file, filtering=filtering, stop_words=stop_words)
...
def scrape_random_contracts(data_dir: str, max_contracts=10000,
    ...
    for contract in contracts:
        ...
        provisions_doc = scrape_exhibit_10(
            contract, filtering=filtering, stop_words=stop_words)
        if provisions_doc:
            provisions.extend(provisions_doc)
            contracts_scraped += 1
            if contracts_scraped == max_contracts:
...
if __name__ == '__main__':
    ...
    provisions = scrape_by_year(data_dir, years=range(
        2019, 2015, -1), stop_words=stop_words)
